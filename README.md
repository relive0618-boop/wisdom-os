...
在 Supabase 先执行已批准的 migration。`pnpm seed:supabase` 是完全离线的预检：不读取环境变量、不建立 Supabase client、不会发出网络请求或写入资料。实际 runner 位于拥有 `@supabase/supabase-js` 的 `apps/web` package，避免 pnpm workspace 的跨 package 解析依赖。只有在 Preview 的服务器环境已安全配置且获得独立批准后，才明确执行 `pnpm seed:supabase:apply`。runner 先以 service-role 做只读 preflight：仅允许补入缺少的 canonical `knowledge_entries` 与 `case_entries`，且新列固定为 `published`、active、无 actor。完全相同的既有 system 列会跳过；内容漂移、Admin 管理列、非 published 或已删除列会以安全错误码停止，绝不覆盖、复活或删除资料。两张表不是跨表 transaction，若后段失败会清楚标记部分成功；重跑只会验证相同 system 列，不产生新版本或 audit event。迁移会开启 RLS：用户只能访问自己的报告和 PDCA；公开内容只读取 `published`；管理员角色只取自 JWT `app_metadata.role`。同步永远由使用者在 `/sync` 明确开始，每批最多 25 笔。下载只还原本机缺少的报告与 PDCA；同 ID 的云端资料绝不覆盖或重复写入本机，而是标示为已有对应本机资料。同步计划会以同一份 canonical payload hash 比对本机、云端与最后一次可信同步状态：未知的同 ID 且内容不同会进入真正冲突，绝不会静默覆盖。每一批的成功 revision 会立即保存；网络失败、冲突或在批次边界取消都保留既有可信 revision、hash 与时间，重新继续时只发送尚未完成的项目。成功的 delete 使用无 body 的 HTTP 204 回应，避免资料库已成功但回应构造失败。

2026-07-26 的人工 Diff 审查发现原先同步写入仍有「先读 revision、后写入」的并发窗口，且 Wizard／Conflict／Admin UI 未完全接线。修复采用新增的 `20260726_wisdom_os_sync_concurrency_hardening.sql`：私有 report／PDCA 的 create、update 与 soft delete 改由 owner 从 `auth.uid()` 派生、带 expected revision 的受控 RPC 原子执行；浏览器不再有直接 DML grant。该 migration 目前只在本机从零重建验证，尚未套用 Preview；本轮的 DELETE 回应与取消／续传 metadata 修复同样只完成本机验证。PR #3 保持 Draft，需另行批准 Preview migration 与真人复验。Production 未操作。

持久化 rate limit 是可选项：服务器先验证 Vercel forwarded IP、再以带 route namespace 的 HMAC-SHA256 匿名化后才传到数据库，数据库不保存原始 IP；RPC 回传、客户端建立或数据库异常都会安全降级为记忆体限流。数据库 bucket 仅保存受限计数，并由 15 分钟 TTL 与专属清理 job 回收；`RATE_LIMIT_HASH_SECRET` 仅在服务器读取，至少 32 字元，绝不能使用 `NEXT_PUBLIC_` 前缀。详细作业说明见 `docs/`。
