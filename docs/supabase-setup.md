...
`20260720100901_wisdom_os_rate_limit_hardening.sql` 已套用至 Preview。它把 rate-limit bucket 改为每个 HMAC identifier／`/api/analyze` 最多一行，跨分钟由受限 RPC 原子重设；旧的非法、重复、超过 11 次或超过 15 分钟未更新的 bucket 会在 migration 内安全收敛。首次 Preview 套用曾在不相容的裸 `LOCK TABLE` 前安全停止，远端未改变；随后把收敛段改为不含显式 `BEGIN`／`COMMIT` 的 transaction-safe `DO` block，第二次套用成功。两次从零本机重建、带旧资料的实测收敛、11+1 计数上限、清理函数与唯一 cron job 都已通过。Preview 只读 verification 已确认 RLS、最小 grants、固定 search path、唯一 active Cron、无 raw IP 与无异常 bucket；同一服务器窗口的受控 10+1 验收为前十次 HTTP 200、第十一次 HTTP 429／`RATE_LIMITED`，TTL 后 Cron 已自动清除验收 bucket。它建立 `updated_at` index、仅清理该表的 `SECURITY DEFINER` 函式，以及每五分钟执行该函式的专属 `pg_cron` job。该表仍启用 RLS，anon／authenticated 没有 table grant，`consume_rate_limit` 仍只授予 service_role，而 cleanup function 没有 direct execute grant。`docs/v0.4-rate-limit-verification.sql` 仅包含 SELECT，不输出 hash 值或 cron job 清单。
