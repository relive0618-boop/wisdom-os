...
`20260720100901_wisdom_os_rate_limit_hardening.sql` 已加入代码库，但尚未套用 Preview。它把 rate-limit bucket 改为每个 HMAC identifier／`/api/analyze` 最多一行，跨分钟由受限 RPC 原子重设；旧的非法、重复、超过 11 次或超过 15 分钟未更新的 bucket 会在 migration 内安全收敛。收敛段使用不含显式 `BEGIN`／`COMMIT` 的 `DO` block，避免 migration runner 对裸 `LOCK TABLE` 的交易相容性问题；两次从零本机重建、带旧资料的实测收敛、11+1 计数上限、清理函数与唯一 cron job 都已通过。先前 Preview 套用安全停止，远端 migration、schema、cron 与业务资料均未改变；须在独立批准后才可重试。它建立 `updated_at` index、仅清理该表的 `SECURITY DEFINER` 函式，以及每五分钟执行该函式的专属 `pg_cron` job。该表仍启用 RLS，anon／authenticated 没有 table grant，`consume_rate_limit` 仍只授予 service_role，而 cleanup function 没有 direct execute grant。套用前请审阅 `docs/v0.4-rate-limit-verification.sql`；它只包含 SELECT，不输出 hash 值或 cron job 清单。Cron 尚未在 Preview 真实验证。
