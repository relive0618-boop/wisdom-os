...
test("未配置時 login 維持安全停用", async ({ page }) => { await page.goto("/login"); await expect(page.getByRole("button", { name: "登入", exact: true })).toBeDisabled(); await expect(page.getByRole("button", { name: "使用 Magic Link 登入", exact: true })).toBeDisabled(); });
