...
export async function audit(client: NonNullable<Awaited<ReturnType<typeof adminContext>>["client"]>, actorId: string, action: string, type: string, id: string) { await client.from("admin_audit_logs").insert({ actor_id: actorId, action, entity_type: type, entity_id: id, metadata: { action } }); }
